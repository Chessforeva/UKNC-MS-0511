...
var StandardRGBColors = Rgb32( /*16 x 8 */ StandardUkncColors );

	function BlackBlueSwap(A) {
		var B = [];
		for(var i in A) {
			var v = A[i];
			switch(v) {
			case 0x000000: v=0x0000FF; break;
			case 0x0000FF: v=0x000000; break;
			case 0x0000DF: v=0x000020; break;
			}
			B.push(v);
		}
		return B;
...
// Table of swapped Black - Blue (some games look better)
var RGblackColors = Rgb32( /*16 x 8 */ BlackBlueSwap( StandardUkncColors ) );

	function invert_colors(A) {
		var B = [];
		for(var i in A) {
			var v = A[i];
			v = ((~v) & 0xFFFFFF)>>>0;
...
var InvertedColors = Rgb32( /*16 x 8 */ invert_colors( StandardUkncColors ) );
...
var GrayColors = Rgb32( /*16 x 8 */ copy8(
  ...
  this.setCol = function(scheme) {
	...
	case 1: colors = RGblackColors; break;
	case 2: colors = InvertedColors; break;
